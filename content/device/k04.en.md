...
Each half of K:04, K:04 Mini, and K:04 Micro has an RGB indicator. Brightness and colors are configured in [Entropy](https://entropy.tools). Set **LED brightness** to 0 to turn the indicators off
...
{{< callout type="info" >}}
  Touchpad gestures are disabled in the firmware by default. To enable them, go to [Entropy](https://entropy.tools), open **Config** - **Modules settings**, and enable **Touch gestures**
{{< /callout >}}
  {{< /tab >}}
  {{< tab >}}
...
- after replacing one module with another, select it in [Entropy](https://entropy.tools) under **Config** - **Modules settings**
...
Use the [Entropy](https://entropy.tools) graphical configurator to remap keys and configure the keyboard

{{< details title="Module configuration" closed="true" >}}

To use the encoder, touchpad, or trackball module, enable it in the keyboard settings. Open the [Entropy](https://entropy.tools) configurator, select **Config** - **Modules settings** at the top of the main configurator window, choose the required half under **Module side**, and choose the connected module under **Module**

![K:04 - module configuration](/images/k04_series/k04_modules_settings_en.gif)

{{< callout type="info" >}}
  If you do not plan to use modules, we recommend disabling them in [Entropy](https://entropy.tools) to increase keyboard battery life
{{< /callout >}}
